...
<style>
    ...
    .metric-title { color: #616161; font-size: 0.9rem; font-weight: 600; margin-bottom: 8px; }
    .metric-value { color: #212121; font-size: 1.6rem; font-weight: 800; }
    .metric-delta.red { color: #f44336; font-size: 1.0rem; font-weight: bold; }
    .metric-delta.blue { color: #2196f3; font-size: 1.0rem; font-weight: bold; }
</style>
...
if "login_info" not in st.session_state:
    ...
    def render_report(item, key_suffix):
        ...
        # 🌟 핵심 방어 로직: 분석 결과가 dict가 아닐 경우를 대비
        an_raw = analyzer.analyze(item['code'], item['name'], "기타", data)
        an = an_raw if isinstance(an_raw, dict) else {}
        ...
        # 안전하게 값 추출
        rsi_val = an.get('rsi')
        rsi_display = f"{rsi_val:.1f}" if (rsi_val is not None and isinstance(rsi_val, (int, float))) else "N/A"
        ...
        with c3:
            st.markdown(f"""<div class="metric-card"><div class="metric-title">RSI(14)</div><div class="metric-value">{rsi_display}</div></div>""", unsafe_allow_html=True)
        with c4:
        ...
        st.info(f"💡 **AI 분석:** {an.get('recommendation', '분석 데이터가 부족합니다.')}")
        ...
        if not is_fav:
            if st.button("➕ 저장하기", key=f"add_{item['code']}_{key_suffix}", use_container_width=True, type="primary"):
    ...
    # --- 탭 로직 ---
    with tab1:
        query = st.text_input("종목명/티커 입력", placeholder="예: 삼성전자, 테슬라")
        if query:
    ...
    with tab2:
        if not st.session_state.custom_stocks: st.info("저장된 종목이 없습니다.")
        else:
            for i, s in enumerate(st.session_state.custom_stocks):
                c1, c2, c3 = st.columns([6, 2, 2])
                c1.write(f"**{s['name']}** ({s['code']})")
                if c2.button("분석", key=f"fav_an_{i}"): render_report(s, f"fav_{i}")
    ...
    with tab3:
        st.write(f"계정: **{user['nick']}** (구글 시트 동기화 중)")
